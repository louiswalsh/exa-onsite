...
export class TrialService {
  ...
  constructor(
    ...
    this.devMode = this.env.isDevMode();
  ...
  getTrialsBySpecialty(specialty: string): Observable<Trial[]> {
    if (this.devMode) {
      return this.mockDataService.getTrialsBySpecialty(specialty);
    }
  ...
  private mapBackendTrialToFrontEnd = (item: any): Trial => {
    ...
    const recruitmentSignal: Trial['recruitmentSignal'] = enrollmentPct >= 80
      ? 'strong'
      : enrollmentPct >= 50
        ? 'moderate'
        : enrollmentAchieved > 0
          ? 'weak'
          : 'failed';
